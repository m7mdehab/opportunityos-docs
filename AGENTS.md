...
- `docs/CI_EFFICIENCY_POLICY.md` defines the standing hosted-runner and Codespaces/local cost discipline.
...
4. Read `docs/CI_EFFICIENCY_POLICY.md` before changing workflows, runner classes, schedules, or test placement.
...
## GitHub Actions and Codespaces Cost Discipline

`docs/CI_EFFICIENCY_POLICY.md` is authoritative for workflow economics. Preserve the same acceptance predicates while using the least expensive capable execution surface.

- Use local development or Codespaces for iterative edit-test-debug cycles when the same toolchain is available; GitHub Actions is the independent merge proof rather than the first debugging loop.
- Prefer coherent checkpoint pushes over micro-pushes that repeatedly trigger the same hosted workflows.
- Keep the Mandatory backend/web suite on a full runner because its PostgreSQL/browser/build workload is not a slim-runner workload.
- Keep Guard, State, Mirror, and Heartbeat on `ubuntu-slim` while their current Git/Python/SSH workloads remain within the slim runner's capability and 15-minute ceiling.
- Keep superseded read-only validation cancelable. Mirror and Heartbeat deliberately retain ordered/non-cancelled write semantics.
- Do not reintroduce full hosted runners for lightweight jobs or add scheduled Actions without documenting a capability/SLA reason and monthly run-cost estimate.
- Do not move private truth or private runtime state into the public mirror to exploit public Actions economics.
...
- CI economics and execution-surface policy: `docs/CI_EFFICIENCY_POLICY.md`
